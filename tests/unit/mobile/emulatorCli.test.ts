...
describe('mobile emulator CLI', () => {
	...
	test('requires an explicit immutable release before Expo Android acceptance', async () => {
		const subprocess = Bun.spawn(
			[
				process.execPath,
				resolve(ROOT, 'src/cli/index.ts'),
				...
				resolve(
					ROOT,
					'tests/fixtures/expo-android-release/absolute.config.ts'
				),
				'--release',
				'--json'
			],
		...
		expect(stderr).not.toContain('not available for the Expo engine');
		expect(stderr).not.toContain('TypeError:');
	});
