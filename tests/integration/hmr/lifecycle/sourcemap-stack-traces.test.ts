...
describe('SSR error logging reaches dev-server stderr', () => {
	test('a thrown error in a Vue SFC is logged with the throw site visible in stderr', async () => {
	...
	test('a thrown error in a Svelte SFC is logged with the throw site mapped to the .svelte source', async () => {
		const { client: c, server: srv } = await startAll();
	...
	test('a thrown error in an Angular component frame points at the on-disk intermediate JS', async () => {
