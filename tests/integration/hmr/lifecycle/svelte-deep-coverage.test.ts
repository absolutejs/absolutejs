...
describe('Svelte 5 deep coverage', () => {
	test('`$state` rune initial value change reaches SSR', async () => {
	...
	test('`$derived` rune recomputes on dependency change', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('composable (.svelte.ts module) body change propagates to importer', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('composable shape change (new exported function) is consumed by importer', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('new component prop (`export let`) is consumed by parent', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('`setContext`/`getContext` flows value from ancestor to descendant', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('`{#if}` block renders the truthy branch', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('`{#each}` block renders every iteration', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('`{#await}` resolved-branch body renders SSR-side', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('`<style>` block edit lands a fresh rule in the SSR-inlined `<style>` block', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('on:click handler edit (Svelte 5 syntax) is invoked correctly during SSR markup pass', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
