...
describe('Svelte 5 deeper coverage', () => {
	test('`$effect.pre` runs synchronously during render (SSR-visible side effect)', async () => {
	...
	test('`$bindable()` prop declaration round-trips through HMR + SSR', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('`{#snippet}` declaration + `{@render}` invocation renders the snippet body', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('`<script context="module">` exports compile and don\'t break SSR', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('`use:action` directive declaration round-trips through HMR + SSR', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('`createEventDispatcher` declaration in a child component round-trips', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('`transition:fade` directive declaration compiles without error', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('`:global()` style selector edit lands a non-scoped rule in served CSS', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('multi-style-block SFC (regular + scoped) edits both land in served CSS', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('`bind:value` directive declaration round-trips through HMR + SSR', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('edit re-emits a fresh hashed page bundle (index URL rotates)', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');

		const beforeHtml = await (await fetch(`${srv.baseUrl}/svelte`)).text();
		...
		const afterHtml = await (await fetch(`${srv.baseUrl}/svelte`)).text();
	...
	test('SSR HTML preserves the Svelte root marker `__absolute_svelte_root__`', async () => {
		const srv = await startAndConnect();
		const html = await (await fetch(`${srv.baseUrl}/svelte`)).text();
		expect(html).toContain('id="__absolute_svelte_root__"');
	}, 30_000);

	test('editing a non-page component re-emits a fresh server bundle (Counter → /svelte)', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('creating + importing a new `.svelte.ts` module mid-session propagates to SSR', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
