...
describe('HTML deeper coverage', () => {
	test(
		'relative `<link rel="stylesheet">` rewrites to manifest-hashed `/indexes/...` URL',
		async () => {
			...
			// CSS URL.
			expect(html).toMatch(
				/href="\/indexes\/html-example\.[a-z0-9]+\.css"/
			);
	...
	test(
		'absolute `/assets/...` href passes through unchanged (favicon path)',
		async () => {
			const srv = await startAndConnect();
			const html = await (await fetch(`${srv.baseUrl}/html`)).text();
	...
	test(
		'HTML page body edit propagates to SSR within one rebuild cycle',
		async () => {
			const srv = await startAndConnect();
	...
	test(
		'multiple `<link>` and `<script>` tags all rewrite correctly',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');
			...
			expect(html).toContain(
				'href="/assets/png/absolutejs-temp.png"'
			);
		},
		60_000
	);
	...
	test(
		'HMR client is injected into served HTML in dev mode',
		async () => {
			const srv = await startAndConnect();
			const html = await (await fetch(`${srv.baseUrl}/html`)).text();
	...
	test(
		'TypescriptExample compiled script lands in the build/ output (not the HTML body)',
		async () => {
			const srv = await startAndConnect();
	...
	test(
		'new HTML page file created mid-session: route still hits the manifest fallback (no crash)',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');
	...
	test(
		'HTML page edit preserves the dev injected `<script type="module" src="/@hmr/...">` link',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');
	...
	test(
		'manifest key for HTML page is the basename (no "Page" suffix)',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');
	...
	test(
		'HTML page CSS path rewrites every time the markup is rebuilt',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');
	...
	test(
		'HTML page broadcast contains framework metadata and body content',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');

			mutateFile(htmlPage, (c) =>
				c.replace('<h1>', '<h1>METADATA_OK ')
			);
	...
	test(
		'public/* files are served at `/<filename>` (mirror)',
		async () => {
			const srv = await startAndConnect();
