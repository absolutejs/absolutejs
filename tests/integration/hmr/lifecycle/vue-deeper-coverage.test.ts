...
describe('Vue deeper coverage', () => {
	test('change-type: template-only edit fires a Vue HMR cycle (vue-update broadcast)', async () => {
		...
		expect((update.data as { framework?: string })?.framework).toBe('vue');

		const html = await waitForBundleAndFetch(client, srv);
		expect(html).toContain('TEMPLATE_ONLY');
	}, 60_000);

	test('change-type: script edit fires a Vue HMR cycle with forceReload semantics', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('`watch` callback runs on initial server render when `immediate: true`', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('`watchEffect` runs synchronously in setup() and its body reaches SSR', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('`defineEmits` declaration round-trips through HMR + SSR without error', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('`v-model` two-way binding declaration via `defineModel` SSR-renders the initial value', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('`v-show` toggles the inline `display` style based on the predicate', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('`@event.modifier` handlers compile and SSR-render correctly', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('multi-style-block SFC (scoped + global) edits land in served CSS', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('custom directive with `getSSRProps` renders the SSR attributes', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('Teleport with `disabled` SSRs the children at their declared spot', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('`Suspense` resolves the default slot SSR-side (no async children)', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('`onServerPrefetch` value is awaited and reaches SSR', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
