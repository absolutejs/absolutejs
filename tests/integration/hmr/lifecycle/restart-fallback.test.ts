...
describe('[abs:restart] stdout marker fires on non-applicable config changes', () => {
	test(
		'non-framework key edit (`buildDirectory`) emits [abs:restart] with "non-framework keys" log',
		async () => {
			...
			await server.waitForOutput(
				/\[abs:restart\] .*absolute\.config\.ts/
			);
		},
		30_000
	);
	...
	test(
		'framework-dir rename emits [abs:restart] with "removed framework(s)" log',
		async () => {
			server = await startDevServer();
