...
describe('HTMX deeper coverage', () => {
	...
	test('HTMX page edit propagates via `htmx-update` HMR broadcast', async () => {
		...
		mutateFile(htmxPage, (c) => c.replace('<h1>', '<h1>HTMX_EDIT_OK '));
		const update = await client.waitFor('htmx-update', 30_000);
		expect((update.data as { framework?: string })?.framework).toBe('htmx');
		const html = await (await fetch(`${srv.baseUrl}/htmx`)).text();
	...
	test('fragment endpoint supports `hx-swap="outerHTML"` style payloads (raw HTML body)', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('fragment endpoint supports `hx-swap-oob` (out-of-band) markup', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('HTMX page broadcast payload contains the edited body content', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
