...
describe('Angular modern template syntax', () => {
	test(
		'`@if` branch edit re-renders the chosen body',
		async () => {
	...
	test(
		'`@for` block edit renders every iteration',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');
	...
	test(
		'`@switch`/`@case` block edit picks the matching case',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');
	...
	test(
		'`@defer` block ships the lowered placeholder marker in SSR',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');
	...
	test(
		'`signal()` initial value change reaches SSR',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');
