...
describe('Vue deep coverage', () => {
	test(
		'`ref()` initial value change reaches SSR',
		async () => {
	...
	test(
		'`computed()` body change reaches SSR',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');
	...
	test(
		'`reactive()` object property change reaches SSR',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');
	...
	test(
		'composable (.ts) body change propagates through to SSR via the importing .vue',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');
	...
	test(
		'composable shape change (new return field) propagates',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');
	...
	test(
		'new prop on child component is consumed by parent and rendered',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');
	...
	test(
		'`provide`/`inject` flows the value from ancestor to descendant',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');
	...
	test(
		'`v-if` directive edit toggles which branch renders',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');
	...
	test(
		'`v-for` directive renders every iteration',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');
	...
	test(
		'`defineExpose` member surfaces no SSR errors when added',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');
	...
	test(
		'scoped `<style scoped>` edit hashes a new data-v attribute into SSR',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');
			...
			const css = await (
				await fetch(`${srv.baseUrl}${match![1]}`)
			).text();
