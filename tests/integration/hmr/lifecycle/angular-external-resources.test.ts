...
const appStyle = resolve(
	PROJECT_ROOT,
	'example/styles/app.component.css'
);
...
describe('Angular external resource edits', () => {
	test(
		'`styleUrl` (.css) edit reaches SSR inlined `<style ng-app-id>` block',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');

			mutateFile(counterStyle, (c) =>
				c.replace('background-color: #1a1a1a;', 'background-color: #ff44aa;')
			);
	...
	test(
		'`@import` chain inside a `styleUrl` propagates the leaf change',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');
			...
			createFile(
				leafStyle,
				`.counter-value { color: #aa44ff; }\n`
			);
			mutateFile(counterStyle, (c) =>
				`@import "./counter.imported.css";\n${c}`
			);
	...
	test(
		'`styleUrl` deep edit (cascade-affecting selector) reaches SSR',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');
	...
	test(
		'`encapsulation: None` style edit propagates without `_ngcontent` rewrites',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');
	...
	test(
		'inline `styles: [...]` array edit reaches SSR',
		async () => {
			const srv = await startAndConnect();
			if (!client) throw new Error('client missing');
