...
describe('Cross-cutting HMR reliability', () => {
	test('long-running session: 25 sequential vue page edits all converge', async () => {
	...
	test('rapid concurrent edits converge on the last-edit state', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('build-error recovery: introduce syntax error, then fix, SSR recovers', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('HMR client disconnect → reconnect: server state preserved across reconnect', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
		...
		await reconnected.waitFor('vue-tier-zero-ssr-rebuild-complete', 30_000);
	...
	test('angular template edit during long-running session does not corrupt manifest', async () => {
		const srv = await startAndConnect();
		if (!client) throw new Error('client missing');
	...
	test('module-server `/@src/` URL serves source files with the dev cache headers', async () => {
		const srv = await startAndConnect();
