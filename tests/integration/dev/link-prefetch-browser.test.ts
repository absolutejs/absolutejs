...
import { runReadyPage } from '../../helpers/browser';
	...
	test('warms the document, the route data and the page modules', async () => {
		...
		await runReadyPage(
			...
			async (page) => {
				...
				expect(
					preloadedBeforeHover.some((href) =>
						href.includes('VueExample')
					)
				).toBe(false);

				observed.length = 0;
				await page.hover('#vue-link');
				// The hover trigger is debounced (250ms); the modulepreload tag
				// can only appear once route data has been parsed.
				await page.waitForFunction(
					() =>
				...
				expect(
					vueRequests.every(({ method }) => method === 'GET')
				).toBe(true);
				...
				expect(
					modules.some((href) => href.includes('VueExample'))
				).toBe(true);
				const styles: string[] = await page.evaluate(
					linkHrefs('prefetch')
				);
				expect(styles.length).toBeGreaterThan(0);
				expect(styles.every((href) => href.endsWith('.css'))).toBe(
					true
				);
				...
				expect(
					loadedModules.some((name) => name.includes('VueExample'))
				).toBe(true);
			}
		);
	}, 180_000);
