...
const ENABLED = process.env.ABSOLUTE_TEST_NATIVE_EXPO_IOS_UPDATES === '1';
const describeNative = ENABLED ? describe : describe.skip;
const PROJECT_ROOT = resolve(import.meta.dir, '..', '..');
const FIXTURE_ROOT = resolve(PROJECT_ROOT, '.absolutejs/expo-ios-update');
...
const APP_ID = `com.absolutejs.expoiosupdateacceptance${Date.now()}`;
...
describeNative('real Expo iOS OTA conformance', () => {
	test('updates, rejects unsafe releases, recovers, and preserves native state', async () => {
		...
		if (process.platform !== 'darwin')
			throw new Error(
				'Expo iOS OTA acceptance requires macOS and Xcode.'
			);
		const xcrun = 'xcrun';
		const installation = await installAbsoluteExpoIosRelease({
			config,
			xcrun,
			log: (line) => console.log(line)
		});
		const { udid } = installation;
		const launch = () => {
			command(
				xcrun,
				'simctl',
		...
		const terminate = () => {
			...
			if (result.exitCode !== 0)
				throw new Error('Expo iOS process termination failed.');
		};
		...
		const publish = async (
			...
			const { ios } = exported.descriptor.platforms;
			if (!ios) throw new Error('Expo iOS export descriptor is missing.');

			return {
				expectedFiles: ios.assets.length + 1,
				launchAssetPath: ios.launchAsset.path,
				manifest: update.manifest,
		...
		await writeFile(
			resolve(ARTIFACT_ROOT, 'expo-ios-update-conformance.json'),
			`${JSON.stringify(
				{
