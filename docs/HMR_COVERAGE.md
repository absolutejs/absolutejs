...
| Real Capacitor WebView on the managed API 36 emulator applies Angular, React, Vue, Svelte, HTML, HTMX and CSS, reports Ember reload fallback, recovers its overlay, and survives background/relaunch | Opt-in [`android-hmr-conformance.test.ts`](tests/native/android-hmr-conformance.test.ts) via `bun run test:native:android` |
| Real Android server restart/WebSocket recovery, process death, no-op native-cache restart, watched native Java rebuild without Bun restart, and release-projection cleanup | Opt-in [`android-lifecycle-conformance.test.ts`](tests/native/android-lifecycle-conformance.test.ts) via `bun run test:native:android:lifecycle` |
| Real Android in-place APK upgrade preserves app identity, native Auth, Sync SQLite and pending outbox state; compatible N+1/N+2 servers render, N+3 shows typed update-required, and rollback recovers | Opt-in [`android-embedded-bundle-conformance.test.ts`](tests/native/android-embedded-bundle-conformance.test.ts) via `bun run test:native:android:upgrade` |
...
| Cold-start with a syntax error in a page — dev server boots with an empty manifest, file watcher alive, recovery rebuild converges to working state once the user fixes the file | [`lifecycle/cold-start-with-broken-page.test.ts`](tests/integration/hmr/lifecycle/cold-start-with-broken-page.test.ts) |
