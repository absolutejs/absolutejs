...
client launch, native-to-web bridge p95, HMR p95 across Angular, React, Vue,
Svelte, HTML, HTMX, and CSS, process PSS/growth, and native plus embedded-web
accessibility. The gate requires labelled, clickable, 44dp/px controls, heading
and keyboard-focus semantics, a platform-exposed WebView, native Fast Refresh
state retention, automatic native rebuild/reconnect, and sanitized timing
artifacts. It also recovers deterministically from API 36 launch-output gaps,
boot-time System UI dialogs, Expo first-run onboarding, UIAutomator's false
nonzero status, and missed cleanup notifications without weakening the actual
HMR apply proof. Run it from this repository root with
...
| Beacon/errors/replay/observability | Browser errors, breadcrumbs, privacy-masked DOM replay, server ingestion and correlation | Shared web/native retry scheduling and consent/retention policy beyond process diagnostics | `mobile.observability` now provisions all-framework WebView capture, Expo-native load/render boundaries, iOS MetricKit diagnostics, and Android `ApplicationExitInfo`; native process reports use a bounded restart-safe outbox, deterministic sampling, strict client redaction, and the same trusted relay |
...
- Capacitor and Expo shells also provision generated, provider-neutral OS
  collectors with no application-native edits. iOS consumes MetricKit crash,
  hang, CPU, disk-write, and launch diagnostics; Android consumes actionable
  `ApplicationExitInfo` crash, native-crash, ANR, low-memory, resource,
  initialization, and signal exits on API 30 and newer.
- Native process reports are bounded to eight files, survive process restart,
  use deterministic sampling, and are acknowledged only after the trusted relay
  accepts the batch. Delivery retries on app activation and, in the Capacitor
  shell, when the browser runtime reports restored connectivity. Android
...
- Extend the native process outbox contract to shared web/native retry scheduling
  only after retention, user-consent, deletion, and connectivity policy are
  explicit. Do not put crash payloads into ordinary application Sync
  collections.
