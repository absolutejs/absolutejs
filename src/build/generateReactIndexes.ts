...
export const generateReactIndexFiles = async (
	...
	const promises = files.map(async (file) => {
		...
		const content = [
			...
			`\t\t\tconsole.warn('⚠️ Ignoring whitespace-only hydration mismatch in <head> (harmless formatting difference)');`,
			...
			`\t\t\t}`,
			`\t\t}\n`,
			`\t\t// CRITICAL: Use createRoot(document) for components that include <html> tags`,
			`\t\t// This matches the structure used by hydrateRoot(document, ...)`,
			`\t\t// The component includes the full document structure (<html><body>...</body></html>)`,
			`\t\t// so we must render to the document root, not a container inside the body`,
			`\t\t// This prevents "html cannot be a child of script" errors`,
			`\t\tconst root = createRoot(document);`,
			`\t\troot.render(createElement(${componentName}, window.__INITIAL_PROPS__ || {}));`,
			`\t\twindow.__REACT_ROOT__ = root;`,
			`\t\twindow.__HMR_CLIENT_ONLY_MODE__ = true;\n`,
			`\t\tconsole.log('✅ Switched to client-only render mode');`,
			`\t} catch (fallbackError) {`,
			`\t\tconsole.error('❌ Failed to fallback to client-only render:', fallbackError);`,
			`\t\t// If client-only rendering fails, reload the page as last resort`,
			`\t\tconsole.warn('⚠️ Reloading page as last resort...');`,
			...
			`\troot = hydrateRoot(`,
			`\t\tdocument,`,
			`\t\tcreateElement(${componentName}, window.__INITIAL_PROPS__ || {}),`,
			`\t\t{`,
