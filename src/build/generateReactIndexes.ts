...
export const generateReactIndexFiles = async (
	...
	const promises = files.map(async (file) => {
		...
		const content = [
			...
			`\t\t\tconsole.log('🔄 Restoring React state from sessionStorage:', hmrState);`,
			`\t\t\tpreservedState = { ...preservedState, ...hmrState };`,
			`\t\t\t// Clear the saved state after restoring`,
			`\t\t\tsessionStorage.removeItem('__REACT_HMR_STATE__');`,
			`\t\t} catch (e) {`,
			`\t\t\tconsole.warn('⚠️ Failed to parse HMR state:', e);`,
			`\t\t}`,
			`\t}`,
			`}\n`,
			...
			`  document.querySelector('[data-absolute-react-root]') || document.getElementById('root') || document.body || null` ,
			`) : null;`,
			`if (!container) {`,
			`\tthrow new Error('React root container not found in document (looked for [data-absolute-react-root], #root, then body)');`,
			`}\n`,
			`let root;`,
			`try {`,
