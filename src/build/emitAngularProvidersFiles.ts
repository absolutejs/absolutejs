 ...
 *  with the extension elided so Bun's resolver can pick whichever variant
 *  is on disk. The build's separate `compileAngularFileJIT` pass walks the
 *  transitive imports from this `.providers.ts` and writes compiled `.js`
 *  for every reachable `.component.ts` (with `templateUrl` inlined). The
 *  compiled `.providers.js` then has its specifiers rewritten to those
 *  compiled `.js` files, so the page bundle that statically imports this
 *  compiled providers file pulls the inlined chain — never the raw `.ts`
 *  source. */
...
const renderFile = (
	...
	if (providersImport) {
		const outputDir = dirname(outputPath);
		// Import from the user's source path. The build runs
		// `compileAngularFileJIT` over this `.providers.ts` separately,
		// which walks all transitive `.ts` imports and inlines each
		// component's `templateUrl`/`styleUrls` into a compiled `.js`
		// under the angular generated tree. The compiled `.providers.js`
		// the page server bundle imports has its specifiers rewritten
		// to those compiled `.js` files, so there's never a runtime
		// path that pulls a raw component source.
	...
	if (pageRoutes?.hasRoutes) {
		// Import from the page module's source path. Like the global
		// providers binding above, `compileAngularFileJIT` walks the
		// transitive imports from this `.providers.ts` and produces
		// inlined `.js` outputs that the compiled `.providers.js`'s
		// own specifiers point at.
