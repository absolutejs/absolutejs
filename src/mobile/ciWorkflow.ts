...
export const ABSOLUTE_MOBILE_CI_WORKFLOW_FORMAT = 4 as const;
...
const publishingInputs = (
	...
	const fields = [
        ...
        type: string`,
		`      dispatch_id:
        description: Unique promotion correlation ID supplied by the AbsoluteJS CLI
        required: false
        type: string`,
...
const promotionValidationSteps = `      - name: Validate promotion handoff
        ...
        run: |
          ...
          if [[ ! "$ABSOLUTE_PROMOTION_DISPATCH_ID" =~ ^amp_[a-f0-9]{32}$ ]]; then
            echo "Promotion requires a valid AbsoluteJS dispatch correlation ID." >&2
            exit 1
          fi
...
const promotionSharedSteps = (
	platform: 'android' | 'ios'
      ...
      - name: Record bounded promotion context
        shell: bash
        run: |
          bun -e 'const dispatchId = process.env.ABSOLUTE_PROMOTION_DISPATCH_ID; const sourceRunId = process.env.ABSOLUTE_SOURCE_RUN_ID; const promotionRunId = process.env.GITHUB_RUN_ID; if (!dispatchId?.match(/^amp_[a-f0-9]{32}$/) || !sourceRunId?.match(/^[1-9][0-9]*$/) || !promotionRunId?.match(/^[1-9][0-9]*$/)) throw new Error("Invalid promotion run identity"); await Bun.write(".absolutejs/mobile-ci/promotion-context.json", JSON.stringify({ dispatchId, format: 2, platform: ${JSON.stringify(platform)}, promotionRunId, sourceArtifact: ${JSON.stringify(`absolute-mobile-${platform}`)}, sourceRunId }, null, 2) + "\\n");'
...
const androidPromotionJob = (options: PlatformJobOptions) => {
  ...
  promote_android:
    ...
    env:
      ...
      ABSOLUTE_PROMOTION_DISPATCH_ID: \${{ inputs.dispatch_id }}
      ABSOLUTE_RELEASE_CHANNEL: \${{ inputs.channel }}
...
run-name: \${{ inputs.operation == 'promote' && format('AbsoluteJS mobile promotion [{0}]', inputs.dispatch_id) || format('AbsoluteJS mobile {0}', github.event_name) }}
