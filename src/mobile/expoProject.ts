...
const expoPackage = (auth: boolean) => ({
	dependencies: {
		...(auth
			? {
...
const layoutSource = (auth: boolean) =>
...
const expoTsConfig = (projectRoot: string, project: string, auth: boolean) => ({
	compilerOptions: {
		paths: {
			...(auth
				? {
...
export const writeAbsoluteExpoProject = async (
	...
	const files = new Map<string, string>([
		...
		[join(project, 'package.json'), jsonSource(expoPackage(authEnabled))],
		...
		[join(project, 'app', '_layout.tsx'), layoutSource(authEnabled)],
		[
