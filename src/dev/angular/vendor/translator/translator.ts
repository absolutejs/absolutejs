	...
	implements o.ExpressionVisitor, o.StatementVisitor, o.TypeVisitor
{
	...
	constructor(
		...
		this.downlevelTaggedTemplates =
			options.downlevelTaggedTemplates === true;
	...
	visitTemplateLiteralExpr(
		...
		return this.setSourceMapRange(
			this.factory.createTemplateLiteral(
				this.getTemplateLiteralFromAst(ast, context)
			),
			ast.sourceSpan
		);
	}
	...
	visitLiteralMapExpr(ast: o.LiteralMapExpr, context: Context): TExpression {
		const properties: ObjectLiteralProperty<TExpression>[] =
			ast.entries.map((entry) => {
				...
				const isPropertyAssignment =
					entry instanceof o.LiteralMapPropertyAssignment ||
					looksLikeProperty;

				if (isPropertyAssignment) {
					const e =
						entry as unknown as o.LiteralMapPropertyAssignment;
					return {
						kind: 'property',
						propertyName: e.key,
						quoted: e.quoted,
				...
				const spreadEntry = entry as unknown as {
					expression?: o.Expression;
				};
				if (!spreadEntry.expression) {
					const ctorName = (
						entry as { constructor?: { name?: string } }
					)?.constructor?.name;
					const keys = entry
						? Object.keys(entry as object).join(',')
						: '';
					throw new Error(
				...
				return {
					kind: 'spread',
					expression: spreadEntry.expression.visitExpression(
						this,
						context
					)
	...
	visitSpreadElementExpr(
		ast: o.outputAst.SpreadElementExpr,
		context: any
	): TExpression {
		const expression = ast.expression.visitExpression(this, context);
		return this.setSourceMapRange(
			this.factory.createSpreadElement(expression),
			ast.sourceSpan
		);
	}
	...
	visitVoidExpr(ast: o.VoidExpr, context: Context): TExpression {
		return this.factory.createVoidExpression(
			ast.expr.visitExpression(this, context)
		);
	}
	...
	private getTemplateLiteralFromAst(
		...
		return {
			...
			expressions: ast.expressions.map((e) =>
