...
EXTRA_ARGS="$@"
for dir in packages/linux-x64 packages/linux-arm64 packages/darwin-x64 packages/darwin-arm64; do
    ...
    cd "$dir" && bun publish --access public $EXTRA_ARGS && cd - > /dev/null
done
